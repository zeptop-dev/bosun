// Package dns keeps DNS records in step with the node: the panel domain,
// the decoy site and TLS inbound names get A/AAAA records on Cloudflare.
package dns
...
// ZoneFor finds the zone the token manages that covers fqdn: the longest
// suffix that exists (a.b.example.com → b.example.com, then example.com).
func (c *Cloudflare) ZoneFor(ctx context.Context, fqdn string) (string, error) {
	...
	for i := 1; i < len(labels)-1; i++ {
		...
		if _, err := c.ZoneID(ctx, zone); err == nil {
			return zone, nil
		}
	}
	return "", fmt.Errorf("cloudflare: no zone for %s with this token", fqdn)
}

// Result is one record's outcome for the UI.
type Result struct {
	Name   string `json:"name"`
	IP     string `json:"ip"`
...
// EnsureAll points every name at ip4 (and ip6 when given), skipping names
// that are empty or already IP addresses.
func (c *Cloudflare) EnsureAll(ctx context.Context, names []string, ip4, ip6 string) []Result {
	var out []Result
	...
	for _, n := range names {
		...
		seen[n] = true
		zone, err := c.ZoneFor(ctx, n)
		if err != nil {
			out = append(out, Result{Name: n, Action: "skipped", Error: err.Error()})
			continue
		}
		for _, ip := range []string{ip4, ip6} {
			if ip == "" {
				continue
			}
			res := Result{Name: n, IP: ip}
			res.Action, err = c.EnsureAddress(ctx, zone, n, ip)
			if err != nil {
				res.Error = err.Error()
			}
			out = append(out, res)
