...
func (s *Store) UpdateUser(u User) error {
	...
	for i, cur := range s.st.Users {
		...
		cur.SpeedLimitMbps = u.SpeedLimitMbps
...
func (s *Store) buildNode(now time.Time) (*spec.Node, []spec.User) {
	...
	for name, raw := range s.st.Overrides {
...
func (s *Store) Report(ctx context.Context, rep agentproto.Report) (bool, error) {
	...
	for tag, t := range rep.Outbounds {
		if s.st.OutboundTraffic == nil {
			s.st.OutboundTraffic = map[string]spec.Traffic{}
		}
		cur := s.st.OutboundTraffic[tag]
		cur.Up += t.Up
		cur.Down += t.Down
		s.st.OutboundTraffic[tag] = cur
	}
