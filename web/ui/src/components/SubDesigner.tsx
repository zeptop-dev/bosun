...
export function SubDesigner() {
  ...
  const q = useQuery({ queryKey: ['sub-design'], queryFn: () => api.get<Data>('/api/sub-design') })
  ...
  const loadPreset = useMutation({ mutationFn: (key: string) => api.get<Design>(`/api/sub-design/presets/${key}`), onSuccess: (r) => { setD(r); setDirty(true) }, onError: toast.err })
  const save = useMutation({ mutationFn: () => api.put('/api/sub-design', { design: d }), onSuccess: () => { toast.ok(t('common.saved')); setDirty(false); qc.invalidateQueries({ queryKey: ['sub-design'] }) }, onError: toast.err })
  const apply = useMutation({ mutationFn: () => api.post('/api/sub-design/apply', { design: d }), onSuccess: () => { toast.ok(t('subDesign.applied')); setDirty(false); qc.invalidateQueries({ queryKey: ['sub-design'] }); qc.invalidateQueries({ queryKey: ['sub-templates'] }) }, onError: toast.err })
  const doPreview = useMutation({ mutationFn: (format: string) => api.post<{ format: string; text: string }>('/api/sub-design/preview', { design: d, format }), onSuccess: setPreview, onError: toast.err })
