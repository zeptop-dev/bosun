...
function waitForRestart(onBack: () => void) {
  ...
  const tick = async () => {
    tries++
    try { await api.get('/api/me'); onBack(); return } catch { /* still restarting */ }
...
export function UpdateCard({ mb }: { mb?: string }) {
  ...
  const q = useQuery({ queryKey: ['update'], queryFn: () => api.get<UpdateInfo>('/api/update'), retry: false })
  const check = useMutation({ mutationFn: () => api.get<UpdateInfo>('/api/update?force=1'), onSuccess: (d) => qc.setQueryData(['update'], d), onError: toast.err })
  const afterExit = () => { setRestarting(true); waitForRestart(() => { setRestarting(false); qc.invalidateQueries() }) }
  const apply = useMutation({ mutationFn: () => api.post('/api/update/apply'), onSuccess: afterExit, onError: toast.err })
  const rollback = useMutation({ mutationFn: () => api.post('/api/update/rollback'), onSuccess: afterExit, onError: toast.err })
  const restart = useMutation({ mutationFn: () => api.post('/api/restart'), onSuccess: afterExit, onError: toast.err })
  const d = q.data
  return (
    <Card mb={mb}>
