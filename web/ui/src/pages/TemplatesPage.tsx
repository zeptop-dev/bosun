...
export default function SubTemplatesPage() {
  ...
  const q = useQuery({ queryKey: ['sub-templates'], queryFn: () => api.get<Data>('/api/sub-templates') })
  ...
  const save = useMutation({
    mutationFn: (text: string) => api.put('/api/sub-templates', { templates: { ...(q.data?.templates ?? {}), [format]: text === fallback ? '' : text } }),
  ...
  return (
    <>
      <PageHeader title={t('subTemplates.title')} subtitle={managed ? t('subTemplates.managedHint') : t('subTemplates.subtitle')} actions={<SegmentedControl size="xs" value={mode} onChange={setMode} data={[{ value: 'design', label: t('subTemplates.modeDesign') }, { value: 'text', label: t('subTemplates.modeText') }]} />} />
      ...
      {mode === 'text' && <Card>
        <Stack gap="sm">
          ...
          <Group justify="flex-end">
            ...
            <Button size="xs" loading={save.isPending} disabled={managed} onClick={() => save.mutate(body)}>{t('common.save')}</Button>
          </Group>
        </Stack>
      </Card>}
    </>
